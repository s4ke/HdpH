...
import Control.Parallel.HdpH.Internal.Location
       (Node, myNode, error, debug, dbgGRef)
...
createEntry :: AnyType -> Node -> GRefReg -> (GRefReg, GRef a)
...
-- Delete entry 'garbageSlot' from 'reg'; not exported
deleteEntry :: Integer -> GRefReg -> (GRefReg, ())
deleteEntry garbageSlot reg =
  (reg { table = Map.delete garbageSlot (table reg) }, ())
